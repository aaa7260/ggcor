...
add_mantel <- function(mantel,
                       ...
                       ...)
{
  if(!inherits(mantel, "mantel_tbl"))
    stop("'mantel' need a mantel_tbl.", call. = FALSE)
  mantel <- rename_mantel(mantel)
                     ...
                     ...)
{
  link_fun <- function(corr) {
    ...
    if(type != "full")
      on.left <- FALSE
    data <- handle_link_data(df, corr, on.left = on.left, diag.label = diag.label,
